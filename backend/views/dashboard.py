...
def _build_dashboard_data(
    ...
    if credit_df is not None and not credit_df.empty:
        ...
        if risk_col:
            ...
            q_crit = credit_df[risk_col].quantile(0.95)
            q_high = credit_df[risk_col].quantile(0.80)
            q_mod  = credit_df[risk_col].quantile(0.50)
            q_low  = credit_df[risk_col].quantile(0.20)
            
            data["risk_buckets"] = {
                "critical": int((credit_df[risk_col] >= q_crit).sum()),
                "high": int(((credit_df[risk_col] >= q_high) & (credit_df[risk_col] < q_crit)).sum()),
                "moderate": int(((credit_df[risk_col] >= q_mod) & (credit_df[risk_col] < q_high)).sum()),
                "low": int(((credit_df[risk_col] >= q_low) & (credit_df[risk_col] < q_mod)).sum()),
                "review": int((credit_df[risk_col] < q_low).sum())
            }
            ...
            for idx, r in top_risky.iterrows():
                ...
                income = float(r.get("person_income", 50000))
                amount = float(r.get("loan_amnt", 10000))
                pd_ratio = score / 100.0
                base_rate = 5.5
                max_ltv = 0.30
                
                if pd_ratio < 0.10:
                    decision = "Auto-Approve"
                    approved_limit = min(income * max_ltv, amount)
                    interest_rate = base_rate + (pd_ratio * 20)
                elif pd_ratio > 0.40:
                    ...
                    approved_limit = min(income * (max_ltv - 0.1), amount)
                    interest_rate = base_rate + (pd_ratio * 35)
